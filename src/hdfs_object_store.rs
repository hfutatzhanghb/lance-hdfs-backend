...
fn convert_error_up(error: Error) -> object_store14::Error {
    match error {
        Error::Generic { store, source } => object_store14::Error::Generic { store, source },
        Error::NotFound { path, source } => object_store14::Error::NotFound { path, source },
        Error::NotSupported { source } => object_store14::Error::NotSupported { source },
        Error::AlreadyExists { path, source } => {
            object_store14::Error::AlreadyExists { path, source }
        }
        Error::Precondition { path, source } => {
        ...
        Error::NotModified { path, source } => object_store14::Error::NotModified { path, source },
        Error::NotImplemented {
            operation,
            implementer,
        } => object_store14::Error::NotImplemented {
        ...
        error => object_store14::Error::Generic {
            store: "hdfs",
            source: Box::new(error),
        },
    }
