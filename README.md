...
```toml
[dependencies]
lance-hdfs-backend = "0.2.2"
lance = { version = "12", default-features = false }
lance-io = { version = "12", default-features = false }
...
This release targets Lance 12. Keep `lance` and `lance-io` on the same major as
the backend so both sides share one set of storage types; a different major
resolves to two copies of `lance-io` and the provider will not type-check.
